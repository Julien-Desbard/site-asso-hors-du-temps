...
//
// Ce module ne doit **aucun** import : il est chargé aussi bien par le script de seed
// que par les contenus de secours de src/lib/fallbacks.ts, qui ne peuvent pas tirer
// payload.config.ts (et avec lui pg, sharp et Lexical).
//
// Pas de `as const` : les tableaux deviendraient `readonly`, incompatible avec
// le `data` attendu par payload.create().
