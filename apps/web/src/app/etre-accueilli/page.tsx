...
const STEPS = [
  {
    ...
    text: "Vous, un proche ou un travailleur social nous appelez ou nous écrivez. On échange simplement sur votre situation et votre besoin de répit.",
  },
  {
    num: '2',
    tag: 'sans jugement',
    title: 'Faire connaissance',
    text: "Un temps d'échange, par téléphone ou sur place, permet de se rencontrer, de présenter la maison et de vérifier ensemble que l'accueil correspond à votre situation.",
  },
  {
    num: '3',
    tag: 'à votre rythme',
    title: 'Préparer le séjour',
    text: "On fixe ensemble les dates et la durée du séjour. On vous explique le fonctionnement de la maison et la vie quotidienne avec les autres.",
  },
  {
    num: '4',
    tag: 'bienvenue',
    title: 'Le séjour à la maison',
    text: "Vous êtes accueilli pour quelques jours : repos, repas partagés, jardin, balades, moments de calme. Vous participez à la vie commune autant que vous le souhaitez.",
  },
  {
    num: '5',
    tag: 'on reste en lien',
    title: 'Après le séjour',
    text: "À votre départ, on prend le temps d'un dernier échange. La porte reste ouverte : beaucoup reviennent, notamment lors des Dimanches Ensemble.",
  },
];

export default function EtreAccueilliPage() {
  return (
    <>
      <PageHero
          ...
          <div className="steps">
            {STEPS.map((step) => (
              <div key={step.num} className="step">
                ...
                <div className="step-num lnum">{step.num}</div>
                <h3>{step.title}</h3>
                <p>{step.text}</p>
              </div>
            ))}
            ...
            <div className="step" style={{ background: 'var(--cream-warm)', borderStyle: 'dashed' }}>
              ...
              <a className="btn btn-primary" href="/nous-contacter" style={{ marginTop: 14, alignSelf: 'flex-start' }}>Nous contacter</a>
            </div>
          </div>
        </div>
      </section>
